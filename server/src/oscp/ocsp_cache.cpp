...
std::optional<URL> parse_url(const std::string& urlstr)
{
    ...
    {
        url.host = urlstr.substr(protocol_host_sep + 3,
                                 host_target_sep - protocol_host_sep - 3);
...
OCSP_Cache::OCSP_Cache(std::chrono::minutes cache_duration,
                       std::chrono::seconds ocsp_request_timeout)
    : m_cache_duration(cache_duration),
      m_ocsp_request_timeout(ocsp_request_timeout)
{
}

std::vector<uint8_t> OCSP_Cache::getOCSPResponse(
    ...
    const Botan::X509_Certificate& subject)
{
    ...
    if (auto cached_ocsp_response = fromCache(fingerprint);
        !cached_ocsp_response.empty())
    {
    ...
    if (!response_from_server.has_value() ||
        response_from_server->status() !=
            Botan::OCSP::Response_Status_Code::Successful ||
        response_from_server->status_for(issuer, subject,
    ...
    return intoCache(fingerprint, issuer, subject,
                     response_from_server.value());
}

std::optional<Botan::OCSP::Response> OCSP_Cache::fetchOCSPResponse(
    const Botan::X509_Certificate& issuer,
    const Botan::X509_Certificate& subject)
{
    ...
    const auto results = resolver.resolve(
        url->host, url->port.value_or(url->protocol == "http" ? "80" : "443"));
    ...
    http::request<http::vector_body<uint8_t>> http_request(http::verb::post,
                                                           url->target, 10);
...
std::vector<uint8_t> OCSP_Cache::fromCache(const std::string& fingerprint) const
{
    ...
    if (entry == m_cache.end() ||
        entry->second.valid_until < std::chrono::system_clock::now())
    {
...
std::vector<uint8_t> OCSP_Cache::intoCache(
    const std::string& fingerprint, const Botan::X509_Certificate& issuer,
    const Botan::X509_Certificate& subject,
    const Botan::OCSP::Response& response)
{
    ...
    std::cout << "caching OCSP response (" << cache_duration.count()
              << "min) for " << fingerprint << "\n";
    ...
    return [&]
    {
        ...
        new_entry = CacheEntry{.ocsp_response = response.raw_bits(),
                               .valid_until = ref_time + cache_duration};
